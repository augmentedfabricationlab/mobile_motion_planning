...
from mobile_motion_planning.partial_trajectory import (# pylint: disable=wrong-import-position
    calculate_partial_trajectory,
)
from mobile_motion_planning.ik_offline.geometry import (# pylint: disable=wrong-import-position
    Plane,
)
...
class TestCalculatePartialTrajectory(unittest.TestCase):
    ...
    def test_with_base_planes(self):
        ...
        result = calculate_partial_trajectory(
            ...
            _current_pose=self.sample_current_pose,
    ...
    def test_result_structure(self):
        ...
        self.assertEqual(
            len(result["ik_solutions_per_node"]), result["num_nodes_computed"]
        )
    ...
    def test_real_data_random_startpoint_matches_reference_path(self):
        ...
        flange_path = (
            export_dir / "flange_frames.json"
        )  #'_251117_163017_flange_frames.json'
        ...
        max_start = min(len(reference_path_configs), len(flange_planes)) - (
            nodes_to_calculate + 1
        )
        ...
        # Remove first element (7 joints -> 6 joints)
        current_pose = reference_path_configs[start_idx][1:]
        ...
        endtime = time.time()
        print(
            f"Calculated {nodes_to_calculate} nodes in {endtime - starttime:.5f} seconds."
        )
        print(f"Result keys: {result.keys()}")
        print(f"Path length: {result['path_length']}")
        print(f"Number of configurations: {len(result['configurations'])}")
        print(
            f"IK solutions per node: {[len(sols) for sols in result['ik_solutions_per_node']]}"
        )
        if "error" in result:
