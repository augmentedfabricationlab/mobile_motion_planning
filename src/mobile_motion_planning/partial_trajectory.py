...
def _save_ik_payload_if_requested(
    ik_solutions_output_path,
    *,
    ik_solutions_list,
    ...
    try:
        ...
        print(f"Saved IK solutions payload to {ik_solutions_output_path}")
    except OSError as e:
        print(f"Failed to save IK solutions payload to {ik_solutions_output_path}: {e}")
...
def calculate_partial_trajectory(
    ...
    for point_index, (target_plane, base_plane) in enumerate(
        ...
        for rotated_target_plane in rotated_target_planes:
            ik_solutions.extend(
                ik_with_tool_and_base_change(rotated_target_plane, base_plane)
            )
    ...
    _save_ik_payload_if_requested(
        ik_solutions_output_path,
        ik_solutions_list=ik_solutions_list,
        rotation_candidates_per_node=rotation_candidates_per_node,
        num_nodes=num_nodes,
        empty_point_indices=empty_point_indices,
    )
    ...
    try:
        ...
        if shortest_path is None:
            print("Ran PathBuilder but no path found")
            return {
                "configurations": [],
                "path_length": float("inf"),
                "num_nodes_computed": num_nodes,
